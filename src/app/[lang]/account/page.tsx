...
export async function generateMetadata({
  ...
  return {
    title: isEn ? "My account" : "我的账号",
    description: isEn
      ? "Review your profile, your comments, your submissions, and admin moderation tools."
      : "查看当前登录用户信息、我的评论、我的投稿和管理员审核入口。",
  };
}
...
export default async function LocalizedAccountPage({
  ...
  if (!claimsData?.claims) {
    redirect(buildLocalizedPath(lang, "/login?next=/account"));
  }
  ...
  const quickLinks = isEn
    ? [
        ["forum", "Skill comments", "Comment on Skills as the signed-in user."],
        ["upload_file", "Skill uploads", "Upload a zip package, fill in metadata, and send it into review."],
        ["storefront", "Community market", "Browse approved featured and community Skills."],
      ]
    : [
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        {notice === "upload_submitted" && (
          <div className="rounded-2xl border border-secondary/20 bg-secondary/5 px-6 py-4 text-sm text-secondary">
            {isEn
              ? "Your submission was received and is now pending review. It will appear in the community market automatically after approval."
        ...
        {notice === "featured_published" && (
          <div className="rounded-2xl border border-primary/20 bg-primary/5 px-6 py-4 text-sm text-primary">
            {isEn
              ? "The featured Skill was published successfully. You can keep editing, unpublishing, or deleting it from the featured management panel below."
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-4">
              {isEn ? "Your account" : "你的账号"}
            </h1>
            <p className="text-lg text-on-surface-variant max-w-2xl leading-relaxed">
              {isEn
                ? "Review your profile, comment history, submission progress, and any moderation results here."
          ...
          <SignOutButton
            locale={lang}
            className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl border border-outline-variant/40 text-sm font-semibold text-on-surface-variant hover:border-primary hover:text-primary transition-colors"
          />
        </div>
          ...
          <div className="lg:col-span-2 bg-surface-container-lowest rounded-3xl p-8 shadow-ambient border border-outline-variant/20">
            ...
            <div className="space-y-4">
              {[
                ["User ID", user.id],
                ["Email", user.email || "-"],
                [isEn ? "Role" : "角色", profile?.role || "user"],
                [
                  isEn ? "Created at" : "创建时间",
                  profile?.created_at
                    ? new Date(profile.created_at).toLocaleString(dateLocale)
                    : user.created_at
            ...
            <div className="bg-primary text-on-primary rounded-3xl p-6 overflow-hidden relative">
              ...
              <div className="relative">
                ...
                <h2 className="text-2xl font-black mb-3">
                  {isEn ? "Manage your community activity" : "管理你的社区内容"}
                </h2>
                <p className="text-sm opacity-90 leading-relaxed">
                  {isEn
                    ? "Track your comments, submissions, and moderation updates in one place."
